...
def check_postgres() -> bool:
    ...
    try:
        with engine.connect() as conn:
            ...
            if missing:
                logger.warning(f"⚠️ PostgreSQL: The following required tables are missing: {', '.join(missing)}")
...
def check_qdrant() -> bool:
    ...
    try:
        ...
        if settings.QDRANT_COLLECTION not in collections:
            logger.warning(f"⚠️ Qdrant: Collection '{settings.QDRANT_COLLECTION}' is missing.")
            logger.warning("👉 Run Phase 4 vector pipeline to create and populate this collection.")
        else:
...
def check_api_keys() -> bool:
    ...
    if not settings.GROQ_API_KEY:
        ...
        logger.warning("⚠️ API Keys: GROQ_API_KEY format looks suspicious (should start with gsk_).")
    else:
    ...
    if not sarvam_key:
        logger.warning("⚠️ API Keys: SARVAM_API_KEY is missing (required for regional voice synthesis).")
    else:
